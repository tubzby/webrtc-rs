...
static DIAG_NACK_GAP_US: AtomicU64 = AtomicU64::new(0);
...
pub fn record_read(ssrc: u32, seq: u16) {
    ...
    if gap_us > max_gap {
        DIAG_MAX_GAP_US.store(gap_us, Ordering::Relaxed);
    }
    ...
    if read_seq >= last_log + 200 {
        if DIAG_LAST_LOG_SEQ.compare_exchange(last_log, read_seq, Ordering::Relaxed, Ordering::Relaxed).is_ok() {
            ...
            let nack_count = DIAG_NACK_COUNT.load(Ordering::Relaxed);
            let stalls = DIAG_STALL_COUNT.load(Ordering::Relaxed);
            eprintln!("SRTP DIAG ssrc={} read_seq={} write_seq={} max_gap={}ms nacks={} stalls={}",
                ssrc, read_seq, write_seq, max_gap_ms, nack_count, stalls);
        }
    }
    
    // Log large gaps (>10ms)
    if gap_us > 10000 {
        eprintln!("SRTP GAP ssrc={} seq={} gap={}us ({}ms)", ssrc, seq, gap_us, gap_us / 1000);
    }
}

pub fn record_nack(ssrc: u32, missing_start: u16, missing_end: u16) {
    let now = Instant::now().elapsed().as_micros() as u64;
    let write_us = DIAG_WRITE_US.load(Ordering::Relaxed);
    let gap_us = now.saturating_sub(write_us);
    DIAG_NACK_COUNT.fetch_add(1, Ordering::Relaxed);
    
    // Log if NACK is sent for a seq that was accepted very recently (<5ms ago)
    if gap_us < 5000 {
        eprintln!("SRTP NACK EARLY ssrc={} missing={}-{} gap={}us", ssrc, missing_start, missing_end, gap_us);
    }
    
    // Count stalls: NACK sent but read hasn't caught up
    let read_seq = DIAG_READ_SEQ.load(Ordering::Relaxed);
